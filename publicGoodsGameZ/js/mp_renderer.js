...
function SocRenderer(pop, data) {
	...
	this.call = async function (contrib, final_cash, balance) {
		...
		var fcash_arr = [];
		var contrib_arr = [];
		var mem_cnt = $(".dynamic .member_cash").length;
		...
		if (err) {
			console.log("error: call recieved incompatible values");
		...
		for (var i = 0; i < this.size; i++) this.member_cash[i] = fcash_arr[i];
	...
	this.wrap_up_partial = async function () {
		...
		for (var i = this.size; i >= 0; i--) {
			...
			if (i < this.size - 1) {
				for (var j = this.interaction_line[i].length - 1; j >= 0; j--) game_canvas.stage.removeChild(this.interaction_line[i][j]);		
				for (var j = this.interaction_line_act[i].length - 1; j >= 0; j--) game_canvas.stage.removeChild(this.interaction_line_act[i][j]);
				for (var j = this.interaction_line_pos[i].length - 1; j >= 0; j--) game_canvas.stage.removeChild(this.interaction_line_pos[i][j]);
			}
		}
		this.render_state = 1;
	}
...
async function main() {
	...
	hats_t = resources["../publicGoodsGame/assets/images/sandbox_hats.json"].textures;
	...
	game_canvas_DIM = Math.min($("#playground").width(), $("#playground").height()) - ($('#playground').innerWidth() - $('#playground').width()) * 2;
	...
	await $("#playground").append(game_canvas.view);
