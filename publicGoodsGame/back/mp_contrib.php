<?php
	...
	if ($rndResult = $cxn->query($rndQuery)) {
		...
		//
		// rnd_major is updated when payoff occurs, rnd_caller is updated when contrib is given (mp_game.php)
		...
		//
		if ((($rnd_caller != $rnd_major + 1) || ($rnd_major % 2 != 1)) && (!$_SESSION['bypass'])) {
			//
			...
			if ($srndResult = $cxn->query($srndQuery)) {
				...
				$srndResult->close();
				if (($rnd_major % 2 == 1) && ($srnd == $rnd_major + 1)) {
					$_SESSION['rnd'] = $srnd;
					$rnd_caller = $srnd;
				}
				else {
	...
	// 2 => complete, expect cash array; end polling
	//
	...
	if ($return['update'] === 2) {
		...
		if (! $present) {
			...
			$q1 = "SELECT `last_balance` FROM `meta` WHERE `grid` = '$grid';";
			$q2 = "SELECT `idx`, `cash`, `contrib` FROM `$grid`;";
			
			if (($r1 = $cxn->query($q1)) && ($r2 = $cxn->query($q2))) {
