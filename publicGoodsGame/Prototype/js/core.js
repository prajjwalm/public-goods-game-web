...
function Simpleton(vol, hf, cash) {
	...
	this.punishment_ratio = 0.2;			// fraction of cash which will be sacrificed for punishment (only for good simpletons)
	...
	this.opinion_vol = 2.5;					// mf of vol for opinions
	this.max_vol = 0.5;
	this.min_vol = 0.01;
	...
	this.request_punishment = function (society){
		...
		if (this.hf > 0.6) {
			...
			print("<br/>");
			for (var Id in list) {
				list[Id] = this.cash * this.punishment_ratio / (n * society.punishment_cost);
			}
		}
...
function Accountant(hf, cash) {
	...
	this.punishment_ratio = 0.1;		// must be < 0.5
	...
	this.request_punishment = function (society){
		...
		if (list.length > 0) {
			...
			this.opinion[target_id1] += this.cutoff_opinion / 2;
			this.opinion[target_id2] += this.cutoff_opinion / 2;
			target[target_id1] = this.cash * this.punishment_ratio / society.punishment_cost;
			target[target_id2] = this.cash * this.punishment_ratio / society.punishment_cost;
		}
		return target;
	}
					...
					if (this.member[perm[i]].cash > red_list[Id] * this.punishment_cost && this.member[idx].cash > 0){	// if punishment executable
						...
						for (var ii = 0; ii < this.size; ii++) {
							if (this.member[ii].strategy != "x") {
								this.member[ii].opinion[this.member[perm[i]].id] -= (this.member[ii].opinion[Id] - 0.5) * 0.5;
								// so opinions can go upto +- 25% based on punishment decisions
						...
						if (this.member[idx].strategy === "x") {
							await sleep(this.justice_sleep * 5);
		...
		if (mangame_state == "off")	return; 		// the button should also be disabled in this case
			...
			if (activeSoc == null) alert("activeSoc == undefined/null");
				...
				if (activeSoc.member[Hidx[click_cntr]].strategy != "x") {
					alert("Some weird error occured around line 1050");
				}
				else {
