<?php
    ...
    if ( $_SERVER['REQUEST_METHOD']=='GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
		...
		if (preg_match($rgx, $hname) === 1) {
            ...
            $nrnds = rand(12, 16) * 2;
			$initquery = "INSERT INTO `$dbname`.`meta` (`grid`, `host`, `max_rnd`) VALUES ('$proposedGR', '$uuid', $nrnds)";
			...
			if ($cxn->query($initquery)) {
				...
				`ping` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,`update_reqd` BOOLEAN NOT NULL DEFAULT FALSE,
                PRIMARY KEY (`idx`), UNIQUE (`name`), UNIQUE (`uuid`)) ENGINE = InnoDB; ";

                $createquery2 = "CREATE TABLE `$dbname`.`mem_$proposedGR` ( `cash`  BLOB NULL DEFAULT NULL, `contrib` BLOB NULL DEFAULT NULL,
                `balance` DECIMAL(21,3) NOT NULL DEFAULT '0' ,	`rp` BLOB NULL DEFAULT NULL, `rnd` SMALLINT NOT NULL AUTO_INCREMENT,
                `ping` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (`rnd`)) ENGINE = InnoDB; ";
				
				if ($cxn->query($createquery) && $cxn->query($createquery2)) {
					//
